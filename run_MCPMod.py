...
# hydra automates inputting arguments to a program
# can be used to save the parameters
@hydra.main(config_path="config", config_name="run_MCPMod")
def run_continuous_agent(cfg: DictConfig):
    ...
    # initialize wandb (just for recording)
    # wandb is a tool used for visualizing various values
    ...
    for ep in pbar:
        ...
        reward = np.round(R, 2)
        pbar.set_description(f"ep={ep}, reward={reward}, t_total={t_total}")
    # Return results
